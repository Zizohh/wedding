				...
				function () {
					...
					for (var i = 0; i < plugins.maps.length; i++) {
						var zoom =
							...
							styles = plugins.maps[i].hasAttribute("data-styles")
					...
					beforeSubmit: function (arr, $form, options) {
						...
						var form = $(plugins.rdMailForm[this.extraData.counter]),
							inputs = form
								...
								.add($('[data-constraints][form="' + form.attr("id") + '"]')),
							output = $("#" + form.attr("data-form-output")),
							captcha = form
								.find(".recaptcha")
								.add($(".recaptcha[form=" + form.attr("id") + "]")),
							captchaFlag = true;

						if (form.attr("id")) {
							console.log(inputs);
							inputs.add(
								$('[data-constraints][form="' + form.attr("id") + '"]')
							);
			...
			for (var i = 0; i < plugins.slick.length; i++) {
				...
				$slickItem
					.slick({
						...
						autoplaySpeed: $slickItem.attr("data-autoplay-speed")
							? parseInt($slickItem.attr("data-autoplay-speed"))
							: 3500,
